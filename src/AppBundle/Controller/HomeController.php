    ...
    public function notifStatusAction(Request $request)
    {
        ...
        if ($form->isSubmitted() && $form->isValid()) {
            ...
            if ($selected_status->getType()!="quote") {
                    ...
                    if($selected_status->getMedia()->getType() == "link"){
                      $thumbnail=$selected_status->getMedia()->getUrl();
                    }else{
...
public function notifUserPaymentAction(Request $request)
    ...
    {   
        ...
        $form = $this->createForm(new PointType(),$settings);
        $form->handleRequest($request);
        if ($form->isSubmitted() && $form->isValid()) {
            $em->flush();
            $this->addFlash('success', 'Operation has been done successfully');
        }
        return $this->render('AppBundle:Home:points.html.twig',array(
            "form"=>$form->createView()));
    }  
