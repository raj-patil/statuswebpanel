...
class StatusController extends Controller {
   ...
   public function getYoutubeId($url) {
    ...
    public function addYoutubeAction(Request $request) {
        ...
        $form = $this->createForm(new YoutubeType(), $video);
        $em = $this->getDoctrine()->getManager();

        $form->handleRequest($request);
        if ($form->isSubmitted() && $form->isValid()) {
           if($this->checkYoutubeUrl($video->getUrlvideo())){
                    $media = new Media();
                    if ($video->getFile() != null) {
                        $media->setFile($video->getFile());
                        $media->setEnabled(true);
                        ...
                        $media->setTitre($video->getTitle());
                        $media->setUrl("https://img.youtube.com/vi/".$this->getYoutubeId($video->getUrlvideo())."/hqdefault.jpg");
                    ...
                    $video_media->setTitre($video->getTitle());
                    $video_media->setUrl($video->getUrlvideo());
                    $video_media->setExtension("youtube");
                    $video_media->setType("youtube");
                    $video_media->setEnabled(false);
                ...
                $type_error = new FormError("The url its not youtube Link");
                $form->get('urlvideo')->addError($type_error);
            }
        }
        return $this->render("AppBundle:Status:video_add_youtube.html.twig", array("form" => $form->createView()));
    }
        public function editYoutubeAction(Request $request, $id) {
        $em = $this->getDoctrine()->getManager();
        $video = $em->getRepository("AppBundle:Status")->findOneBy(array("id" => $id, "review" => false));
        if ($video == null) {
            throw new NotFoundHttpException("Page not found");
        ...
        return $this->render("AppBundle:Status:video_edit_youtube.html.twig", array("form" => $form->createView()));
    }
                ...
                if (sha1($user->getPassword()) == $userKey) {
                    ...
                    if ($transaction==null) {
                        if ($status->getUser()->getId() != $user->getId()) { 
                            ...
                            $transaction_author = new Transaction();
                            $transaction_author->setPoints($setting->getPoints("author".$type));
                            $transaction_author->setStatus($status);
                            $transaction_author->setUser($status->getUser());
                            $transaction_author->setType("author_".$type);
                            $em->persist($transaction_author);
                            $em->flush();
		...
		if ($user) {
			...
			if ($this->getRequest()->files->has('uploaded_file')) {
                ...
                if ($user->getTrusted()) {
                    $transaction = new Transaction();
                    $setting = $em->getRepository("AppBundle:Settings")->findOneBy(array());
                    $transaction->setPoints($setting->getPoints("addvideo"));
                    $transaction->setStatus($w);
                    $transaction->setUser($user);
                    $transaction->setType("add_video");
                    $em->persist($transaction);
                    $em->flush();
                    $this->sendNotif($em,$w);
                }
    ...
    public function api_uploadYoutubeAction(Request $request, $token) {
        ...
        $key = str_replace('"', '', $request->get("key"));
        $title = str_replace('"', '', $request->get("title"));
        $link = str_replace('"', '', $request->get("link"));
        $description = str_replace('"', '', $request->get("description"));
        ...
        if ($user) {
                ...
                if ($this->getRequest()->files->has('uploaded_file')) {
                    ...
                    $media_thum->setFile($file_thum);
                    $media_thum->setEnabled(true);
                    $media_thum->upload($this->container->getParameter('files_directory'));
                }else{
                    $media_thum->setTitre($title);
                    $media_thum->setUrl("https://img.youtube.com/vi/".$this->getYoutubeId($link)."/hqdefault.jpg");
                    $media_thum->setExtension("jpg");
                    $media_thum->setType("link");
                    $media_thum->setEnabled(false);
                ...
                $video_media = new Media();
                $video_media->setTitre($title);
                $video_media->setUrl($link);
                $video_media->setExtension("youtube");
                $video_media->setType("youtube");
                $video_media->setEnabled(false);
	...
	public function indexAction(Request $request) {
		...
		if ($request->query->has("q") and $request->query->get("q") != "") {
			$q .= " AND  i.title like '%" . $request->query->get("q") . "%'";
		}
