...
foreach ($slides as $key => $slide) {
	...
	if ($slide->getType() == 3 && $slide->getStatus() != null) {
		...
		if ($status_slide->getMedia()->getType() == "link") {
			$a_slide["thumbnail"]= $status_slide->getMedia()->getUrl();
		}else{
...
foreach ($full_screen_videos as $key => $status_full_screen) {
	...
	if ($status_full_screen->getType()!="quote") {
		...
		if ($status_full_screen->getMedia()->getType() == "link") {
			$a_fullscreen["thumbnail"]= $status_full_screen->getMedia()->getUrl();
		}else{
...
foreach ($videos as $key => $status) {
	...
	if ($status->getType()!="quote") {
		...
		if ($status->getMedia()->getType() == "link") {
			$a["thumbnail"]= $status->getMedia()->getUrl();
		}else{
