...
class UserController extends Controller
{
    ...
    public function transactionAction(Request $request,$id)
    {
        ...
        $dql        = "SELECT c FROM AppBundle:Transaction c  WHERE  c.user = ".$user->getId() ." order by c.created asc";
        $query      = $em->createQuery($dql);
        $paginator  = $this->get('knp_paginator');
        $pagination = $paginator->paginate(
            $query,
   ...
   public function api_getAction(Request $request,$user,$me,$token){
        ...
        $values[]=array("name"=>"status","value"=>sizeof($status));
    ...
    public function api_registerAction(Request $request,$token)
    {
        ...
        if ($u!=null) {
            ...
            if (count($errors)==0) {
                ...
                $transaction = new Transaction();
                $setting = $em->getRepository("AppBundle:Settings")->findOneBy(array());
                $transaction->setLabel("You has been registered in the app");
                $transaction->setPoints($setting->getPoints("registeruser"));
                $transaction->setUser($user);
                $transaction->setType("register_user");
                $em->persist($transaction);
                $em->flush();
    ...
    public function statusAction(Request $request,$id)
    {
        ...
        $dql        = "SELECT w FROM AppBundle:Status w  WHERE w.user = ".$user->getId() ." ORDER BY w.created desc";
        $query      = $em->createQuery($dql);
        $paginator  = $this->get('knp_paginator');
        $pagination = $paginator->paginate(
            $query,
        ...
        return $this->render(
                'UserBundle:User:status.html.twig',array(
                    "pagination"=>$pagination,
                    "user"=>$user
                )
            );
    ...
    public function  api_followingstopAction(Request $request,$user,$token)
    {
        ...
        foreach ($user->getUsers() as $key => $e) {
            if (sizeof($e->getStatus())>0) {
               ...
               $b["image"] = $e->getImage();
               $last_wallpaper = $em->getRepository('AppBundle:Status')->findOneBy(array("enabled"=>true,"user"=>$e),array("created"=>"desc"));
